...
Final Population
```{r daphnia final pop}
pop_fin <- popA %>% group_by(TankNum) %>% filter(ExptDay == max(ExptDay))

## maybe this is weird bc treating adult adn juv the same- maybe artifically increasing the number of subsamples
daph_fin_mod <- glmer(count ~ animal + (1|TankNum), subset = (disturb=="y"), data = pop_fin, family = "poisson" )
plot(resid(daph_fin_mod))
qqnorm(resid(daph_fin_mod))

e1_pop <-  emmeans(daph_fin_mod, ~animal, type = "response")
...
dd1 <- left_join(popA,dd, by= "TankNum")
daph_dist <- dd1 %>% filter(ExptDay >= E1)
daph_avg_pop <- daph_dist %>% group_by(TankNum, ExptDay) %>% 
  mutate(pop = mean(count, na.rm= TRUE), sdpop= sd(count, na.rm = T))
daph_max <- daph_avg_pop %>% group_by(TankNum) %>% filter(pop == max(pop))

daph_max_mod <- lm(log(pop) ~ animal*disturb, data = daph_max, weights = sdpop)
