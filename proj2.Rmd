...
sedmod <- lm(log(sediment) ~ animal,data = sed3, subset=(disturb=="y") ) ## hmm do all my models need random effect of tank??-- i only have 2 measurement per tank...so that doesn't seem doable but should i do something? 
...
eggg <- ggplot(data = dd, aes(treat, prob)) + geom_point(aes(color = disturb, shape = animal), size = 3) + geom_errorbar(aes(ymin= asymp.LCL, ymax=asymp.UCL, color= disturb), width = 0.3)+ ylab("Probablity Egg Cases Present") + 
...
popg <- ggplot(data = popgd, aes(treat,rate)) +  
  ...
  xlab("Grazer")+ theme(axis.line = element_line(colour = "black"), panel.border = element_blank()) + scale_color_manual(values = c("black", "seashell4")) + ggtitle("Final Population")+ theme(legend.position = "none") 
...
popg_max <- ggplot(data = popmg, aes(treat,response)) +  
   geom_point(aes(color = disturb, shape = animal), size = 3) + geom_errorbar(aes(ymin= lower.CL, ymax=upper.CL, color= disturb), width = 0.3)+ ylab(" ") + 
  xlab("Grazer")+ theme(axis.line = element_line(colour = "black"), panel.border = element_blank()) + scale_color_manual(values = c("black", "seashell4")) + ggtitle("Maximum population")

grid.arrange(popg, popg_max, nrow = 1)
