...
maxmod <- lm(log10(mean_3) ~ animal, data = max_chl)
...
amm_end <- lm(data = finday1, NH4 ~ -1+ animal)
...
g2 <- ggplot(data = grdat3, aes(animal, emmean)) + geom_point(size = 3) + 
  geom_errorbar(aes(ymin=lower.CL, ymax = upper.CL), width = 0.3) + ylab("Final NH4-N (mg/L)") + 
  xlab("Grazer")+ theme(axis.line = element_line(colour = "black"), panel.border = element_blank()) + ggtitle("C.")
...
nh_max1 <- dat_dist %>% group_by(TankNum) %>% mutate(max_nh4= max(NH4,na.rm = T))
nh_max1$animal <- factor(nh_max1$animal, levels = c("none", "snail", "daphnia", "both"))

nh4_max_mod <- lm(max_nh4 ~ -1+animal, data = nh_max1)
...
## percent difference
## wasn't sure how to do this efficiently took estimates from main model 
## treatment diff/ mean(treatments)
## but not clear how to get CI on this.. have CI on differance (confint(amm_max)$confint)
plot(e1_max)
...
g3 <- ggplot(data = grdat4, aes(animal, emmean)) + geom_point(size = 3) + 
  geom_errorbar(aes(ymin=lower.CL, ymax = upper.CL), width = 0.3) + ylab("Maximum NH4-N (mg/L)") + 
  xlab("Grazer")+ theme(axis.line = element_line(colour = "black"), panel.border = element_blank()) + ggtitle("D.")
...
sedmod <- lm(log10(sediment) ~ -1+ animal,data = sed3, subset=(disturb=="y") ) ## hmm do all my models need random effect of tank??-- i only have 2 measurement per tank...so that doesn't seem doable but should i do something? 
...
g3 <- ggplot(data = grdat5, aes(animal, response)) + geom_point(size = 3) + 
  ...
  xlab("Grazer")+ theme(axis.line = element_line(colour = "black"), panel.border = element_blank())
...
```{r egg case model}
eggmod <- glm(eggs~animal*disturb, family = binomial(link= logit), data = egg4)
...
eggg <- ggplot(data = dd, aes(treat, prob)) + geom_point(aes(color = disturb, shape = animal), size = 3) + geom_errorbar(aes(ymin= asymp.LCL, ymax=asymp.UCL, color= disturb), width = 0.3)+ ylab("Probablity Ephippium Present") + 
  xlab("Grazer")+ theme(axis.line = element_line(colour = "black"), panel.border = element_blank()) + scale_color_manual(values = c("black", "seashell4")) + 
    labs(color="Disturbance", shape = "Grazer") 
```
...
daph_fin_mod <- glmer(total ~ treat + (1|TankNum),data = pop_fin, family = "poisson" )
...
popg <- ggplot(data = popgd, aes(treat,rate)) +  
  ...
  xlab("Grazer")+ theme(axis.line = element_line(colour = "black"), panel.border = element_blank()) + scale_color_manual(values = c("black", "seashell4")) + ggtitle("A. Final Population")+ theme(legend.position = "none") 
...
popg_max <- ggplot(data = popmg, aes(treat,response)) +  
  ...
  xlab("Grazer")+ theme(axis.line = element_line(colour = "black"), panel.border = element_blank()) + scale_color_manual(values = c("black", "seashell4")) + ggtitle("B. Maximum population") + labs(color = "Disturbance", shape = "Grazer") + theme(legend.position = c(0.85,0.25), legend.box = "horizontal", legend.background = element_rect(fill = "transparent"))
