...
```{r ammonium max}
max_day <- dat[which(dat$Disturbed=="y"), ]
nh_max <- dat %>% filter(disturb == "y") %>% filter(ExptDay >= max_day$ExptDay) ## no bc needs to match by tank
...
```{r daphnia max}
daph_dist <- popA %>% filter(ExptDay >= max_day$ExptDay)

```



Growth post disturbance
