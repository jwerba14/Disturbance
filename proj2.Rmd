...
## still wrong what i actually want to understand resilience is the difference between undisturbed and disturbed for particular herbivore combination (daphnia y - daphnia n) > or less than < the difference in another treatment (snail y vs snail n) 
...
daph_fin_mod <- glmer(total ~ -1 + treat + (1|TankNum) + (1|block),data = pop_fin, family = "poisson" ) 
...
daph_fin_nb <- glmer.nb(total ~ -1 + treat + (1|TankNum) + (1|block),data = pop_fin)




pp <- multcomp::glht(daph_fin_nb,inverse_contp)
summary(pp)

e1_pop <-  emmeans(daph_fin_nb, ~treat, type = "response")
